...
## Step 4 — Continue into the dashboard (optional, local-only is fine)
...
To get **live** visibility (the session, the block, and the exact-path approval
as they happen), run the account-backed setup inside the real repository:

```bash
npx -y @neurcode-ai/cli@latest setup --agent claude
```

Setup progresses through browser login, explicit personal or organization
workspace selection, repository ownership, the local Brain, and agent
integration. Re-run the same command to resume. The dashboard clearly labels
whether you are in local-only or connected mode and never presents a dead
bridge as if it were live.

---

## Step 5 — Inspect your real repository (read-only)
...
run the guided read-only evaluation after setup:

```bash
cd your-repository
npx -y @neurcode-ai/cli@latest setup --agent codex       # canonical first run / resume
...
> Lead a no-account evaluator with `pilot start --fixture --agent <agent>` and a
> connected real-repo user with `setup --agent <agent>`.
